...
def _parsed_args():
    ...
    parser.add_argument(
        ...
        '--handle',
        type=int,
        default=0, #todo: for prod revert to 50 
        required=False,
...
def print_parsed_clips(clips: List[CutClip]) -> None:
    """Print parsed clip paths with per-clip and total duration in seconds."""
    if not clips:
        logging.info("No parsed clips found in timeline.")
        return

    total_duration = 0.0
    logging.info("Parsed clips and durations:")
    for idx, clip in enumerate(clips, start=1):
        total_duration += clip.duration
        logging.info("  %d. %s | duration=%.3fs", idx, clip.path, clip.duration)
    logging.info("Total parsed clips: %d", len(clips))
    logging.info("Total duration: %.3fs", total_duration)


def log_cutclip_summary(clips: List[CutClip]) -> None:
    """Log a detailed summary of all CutClip objects before exit."""
    logging.info("\n" + "="*80)
    logging.info("FINAL CUTCLIP SUMMARY")
    logging.info("="*80)
    if not clips:
        logging.info("No clips to summarize.")
        logging.info("="*80 + "\n")
        return
    ...
    for idx, clip in enumerate(clips, start=1):
        ...
        logging.info("  Start: %.3fs", clip.start)
        logging.info("  Duration: %.3fs", clip.duration)
        logging.info("  Output File: %s", clip.output_file)
        logging.info("  Output Dir: %s", clip.output_dir)
        logging.info("  BMX Start Frames: %d", clip.bmx_start_frames)
        logging.info("  BMX Duration Frames: %d", clip.bmx_duration_frames)
        logging.info("  Processing Success: %s", clip.processing_success)
        logging.info("  BMX Cmd: %s", clip.bmx_cmd.cmd if clip.bmx_cmd else "None")
        logging.info("  FFmpeg Cmd: %s", clip.ffmpeg_copy_cmd.cmd if clip.ffmpeg_copy_cmd else "None")

    logging.info("\n" + "="*80)
    logging.info("Total clips: %d", len(clips))
    logging.info("="*80 + "\n")
...
def main():
    ...
    # Surpress otio-aaf-adapter logging — it logs every mob it reads, which is excessive for large timelines. We still want to see our own debug logs, so set the adapter logger to WARNING.
    try:
        import otio_aaf_adapter.adapters.advanced_authoring_format as _aaf_mod
        _aaf_mod._transcribe_log = lambda *args, **kwargs: None
    except Exception:
        pass
    ...
    for _t in result_tl.tracks:
        if _t.kind != "Video":
            continue
        for idx, item in enumerate(_t):
            #
            # For each clip, find original filename, duration and start time (zero based) in order to create bmx/ffmpeg cmd
            # Effects like Dissolve and TimeWarp are taken into account to adjust the duration and start time for bmx, so that the consolidated clip captures the full media that is actually played in the timeline.
            #
            ...
            if isinstance(item, otio.schema.Clip):
                ...
                if sr:
                    ...
                    # Adjacent transitions: adapter shrinks source_range to exclude dissolve overlap for OTIO
                    # conformance; we re-extend so consolidation captures the full media that's actually played.
                    prev_trans_in = otio.opentime.RationalTime(0, rate)
                    next_trans_out = otio.opentime.RationalTime(0, rate)
                    if idx > 0 and isinstance(_t[idx - 1], otio.schema.Transition):
                        prev_trans_in = _t[idx - 1].in_offset
                    if idx + 1 < len(_t) and isinstance(_t[idx + 1], otio.schema.Transition):
                        next_trans_out = _t[idx + 1].out_offset
                    
                    # otio-aaf-adapter >=2.0 shifts source_range into real-TC space by the source mob's
                    # global_start_time; subtract available_range.start_time to zero-base. Targets v2.x only.
                    mr = item.media_reference
                    ar = mr.available_range if mr else None
                    offset = ar.start_time if ar else otio.opentime.RationalTime(0, rate)
                    zero_based_start = sr.start_time - offset - prev_trans_in
                    my_start_time = zero_based_start.value / real_rate
                    
                    # Avid speed filters: source_range.duration is timeline-side; LinearTimeWarp.time_scalar
                    # converts to source-side. Transition extensions are timeline-side too — apply before warp.
                    adjusted_duration = sr.duration + prev_trans_in + next_trans_out
                    if prev_trans_in.value or next_trans_out.value:
                        logging.debug("Transition extensions on %s: prev_in=%s, next_out=%s frames (timeline)",
                                      _path, prev_trans_in.value, next_trans_out.value)
                    for eff in item.effects:
                        if isinstance(eff, otio.schema.LinearTimeWarp):
                            if eff.time_scalar == 0:
                                adjusted_duration = otio.opentime.RationalTime(1, adjusted_duration.rate)
                                logging.debug("FreezeFrame on clip %s — clamping to 1 frame", _path)
                            else:
                                adjusted_duration = otio.opentime.RationalTime(
                                    adjusted_duration.value * eff.time_scalar,
                                    adjusted_duration.rate)
                                logging.debug("LinearTimeWarp time_scalar=%s on %s: %s -> %s frames",
                                              eff.time_scalar, _path, sr.duration.value, adjusted_duration.value)
                        elif isinstance(eff, otio.schema.TimeEffect):
                            logging.warning("Non-linear time warp on %s — duration not adjusted", _path)
                    my_duration = adjusted_duration.value / real_rate
    ...
    log_cutclip_summary(main_clip_list)
